...
| Cost accumulator | Tanmay | done | `server/cost/accumulator.py::CostAccumulator` folds every model call (attempts + repair retries + escalation) into per-problem token/cost totals → `SolveResponse` → `problem_solving` → summed per run by `/metrics` + `analysis_script`. Tested in `scripts/test_cost_accumulator.py`. |
...
## Changelog
- **2026-06-10** — **First live benchmark on the `lab-2` TPU** (`run_id=tpu_bench_001`, see `BENCHMARK_REPORT.md`). Drove 46 problems (math 20 / reasoning 16 / code 10) through the real granite router on the TPU + OpenRouter solvers. **40/45 solved (88.9%); 40/40 (100%) excluding rate-limited**, 0 escalations, total cost ~$3.9e-4. By domain: code 10/10, reasoning 14/15, math 16/20. All non-solves were infrastructure: 5 × OpenRouter free-tier 429 (daily cap) and 1 dropped where the granite router emitted non-JSON (`Mary`). Router chose `gpt-oss-20b` for the whole (easy-skewed) sample — cost-optimal, no over-routing. Found two robustness items to fix: (a) harden the router JSON parse / retry on non-JSON, (b) back off on 429. Validated the run via our own `:8002` server (caught the router `max_completion_tokens=8192` > 8192-ctx bug, now `ROUTER_MAX_OUTPUT_TOKENS`).
- **2026-06-10** — **TPU deploy prep for `lab-2`** (`v5litepod-1` / TPU v5e). Made `run.sh` accelerator-aware: auto-detects a Cloud TPU (`/dev/accel0`), adds `--device tpu`, and maps the `DTYPE=fp8` weight-quant path to `tpu_int8` (v5e has no fp8 weights; needs v6e/Ironwood). `--kv-cache-dtype fp8` is kept (supported on TPU v5+ / vLLM ≥0.10). GPU/mac/Colab behavior is unchanged (guarded behind `ACCEL`, default `auto`; override with `ACCEL=gpu|tpu`). Added a "Run on the `lab-2` Cloud TPU" section to the README. (Live deploy still blocked on the TPU IAM grant for `tanmay@uw.edu`.)
- **2026-06-10** — **Switched the model lineup** to four free OpenRouter models: `openai/gpt-oss-20b:free` (21B/3.6B), `openai/gpt-oss-120b:free` (117B/5.1B), `qwen/qwen3-coder:free` (480B/35B), `nvidia/nemotron-3-ultra-550b-a55b:free` (550B/55B). Replaced `deepseek-v4-flash`→`qwen3-coder` and `kimi-k2.6`→`nemotron-3-ultra` across `configs/models.yaml` + `configs/benchmarks.yaml` (keys kept in sync — router requires it) and every hardcoded reference (`e2e_smoke.py`, `resolution.py` fallback, `local-inference/utils.py` fallback, `analysis_script.py` rank map + routing heuristic, docs). `STRONGEST_MODEL_ID` now auto-derives to `nemotron-3-ultra` (largest by params). Verified: config key parity, 5/5 accumulator tests, 37/37 smoke. Benchmark scores were carried over onto the new keys (placeholder; swap in real Qwen3-Coder/Nemotron numbers when available).
- **2026-06-10** — Confirmed the two requested deliverables are done:
  - **Test benchmarks for Code + Logic/Math eval:** the recognized academic sets are vendored and built into the domains. **Code = HumanEval** (164 tasks, `openai/openai_humaneval`, MIT) → `domains/code.json` (167 incl. 3 base), run as `verify: tests` in the sandbox. **Math = GSM8K** (40 tasks, `openai/gsm8k`, MIT) plus curated arithmetic → `domains/math.json` (50; 49 `numeric_match` heuristic + 1 match). **Logic = 16 curated** logic/sequence puzzles → `domains/reasoning.json`. Provenance + offline vendoring in `problems/import_humaneval.py` / `import_gsm8k.py`; both `--refresh`-able.
  - **Cost accumulator:** promoted to a first-class component — `server/cost/accumulator.py::CostAccumulator`. `solve_problem` now folds every model call (router attempts, solve→run→repair retries, escalation) through `acc.add(call_result)`, accumulating `prompt_tokens` / `completion_tokens` / `image_tokens` / `cost` / `num_calls`. Failed calls count tokens but $0 cost; a bad model id can never break the loop (cost→0). The per-problem total flows into `SolveResponse.total_cost` → `problem_solving` → summed per run by `/metrics` and `analysis_script`. Covered by `scripts/test_cost_accumulator.py` (5 dependency-free unit tests, wired into the CI workflow) and the E2E smoke test (total cost accrues across all domains).
